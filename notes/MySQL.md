...
![sql层的处理流程](\images\sql层的处理流程.png)
...
1. 验证SQL语法和SQL_MODE
   1. sql_mode,简而言之就是：它定义了你MySQL应该支持的sql语法，对数据的校验等等。
   2. 查看当前数据库使用的sql_mode：mysql> ``select` `@@sql_mode;
2. 验证语义
3. 验证权限
...
![sql执行过程](images\sql执行过程.png)

![执行计划获取](E:\smile\db\images\执行计划获取.png)
